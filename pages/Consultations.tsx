...
const ServiceCard: React.FC<ServiceCardProps> = ({ title, price, includes, excludes, note, disclaimer, highlight }) => (
  <motion.div 
    variants={{
  ...
  >
    {highlight && (
      <motion.div 
        ...
        className="bg-[#B4975A] text-white text-[10px] font-bold uppercase tracking-[0.2em] py-1.5 px-4 rounded-full self-start mb-8 shadow-sm"
      >
        Highly Strategic
      </motion.div>
    )}
    {!highlight && <div className="h-4 self-start mb-8"></div>}
  ...
  return (
    <div className="py-12 md:py-32 px-6 bg-[#F8FAFC]">
      <div className="max-w-7xl mx-auto">
        {/* Intro Section - Redesigned Feature Card */}
        <motion.div 
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className="p-8 md:p-16 lg:w-3/5 flex flex-col justify-center">
            ...
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-serif font-bold text-slate-900 mb-6 md:mb-8 leading-tight">Consultations</h1>
            <p className="text-lg md:text-xl lg:text-2xl text-slate-600 font-light leading-relaxed">
              Professional, strategic guidance for high-achieving foreign-trained attorneys navigating the complex path to U.S. bar admission.
            </p>
          </div>

          {/* Desktop Image (large, full height) */}
        ...
        <motion.div 
          className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-32 items-stretch"
          initial="initial"
          whileInView="animate"
          viewport={{ once: true }}
          variants={{
        ...
        >
          ...
          <ServiceCard
            ...
            note="Best for those who already have a study plan."
          />
